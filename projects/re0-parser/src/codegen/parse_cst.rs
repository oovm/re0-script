...
fn parse_root(state: Input) -> Output {
    state.rule(LifeRestartRule::Root, |s| {
        s.sequence(|s|Ok(s).and_then(|s|s.repeat(0..4294967295,|s|s.sequence(|s|Ok(s).and_then(|s|builtin_ignore(s)).and_then(|s|parse_statement(s).and_then(|s| s.tag_node("statement")))))).and_then(|s|builtin_ignore(s)).and_then(|s|s.end_of_input()))
    })
}
#[inline]
...
fn parse_property_statement(state: Input) -> Output {
    state.rule(LifeRestartRule::PropertyStatement, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_kw_property(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_identifier(s).and_then(|s| s.tag_node("identifier"))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s,"{",false)).and_then(|s|builtin_ignore(s)).and_then(|s|s.repeat(0..4294967295,|s|s.sequence(|s|Ok(s).and_then(|s|builtin_ignore(s)).and_then(|s|parse_property_item(s).and_then(|s| s.tag_node("property_item")))))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s,"}",false)))
    })
}
#[inline]
...
fn parse_trait_property(state: Input) -> Output {
    state.rule(LifeRestartRule::TraitProperty, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_identifier(s).and_then(|s| s.tag_node("identifier"))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s,":",false)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_atomic(s).and_then(|s| s.tag_node("atomic"))))
    })
}
#[inline]
fn parse_event_statement(state: Input) -> Output {
    state.rule(LifeRestartRule::EventStatement, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_kw_event(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_identifier(s).and_then(|s| s.tag_node("identifier"))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_event_block(s).and_then(|s| s.tag_node("event_block"))))
    })
}
#[inline]
fn parse_event_block(state: Input) -> Output {
    state.rule(LifeRestartRule::EventBlock, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s,"{",false)).and_then(|s|builtin_ignore(s)).and_then(|s|s.repeat(0..4294967295,|s|s.sequence(|s|Ok(s).and_then(|s|builtin_ignore(s)).and_then(|s|parse_event_property(s).and_then(|s| s.tag_node("event_property")))))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s,"}",false)))
    })
}
#[inline]
fn parse_event_property(state: Input) -> Output {
    state.rule(LifeRestartRule::EventProperty, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_identifier(s).and_then(|s| s.tag_node("identifier"))).and_then(|s|builtin_ignore(s)).and_then(|s|Err(s).or_else(|s|builtin_text(s,":",false)).or_else(|s|builtin_text(s,"[",false))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_expression(s).and_then(|s| s.tag_node("expression"))).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|Err(s).or_else(|s|builtin_text(s,"]",false)).or_else(|s|s.sequence(|s|Ok(s).and_then(|s|builtin_text(s,"{",false)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_event_block(s).and_then(|s| s.tag_node("event_block"))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s,"}",false)))).or_else(|s|parse_event_block(s).and_then(|s| s.tag_node("event_block"))))))
    })
}
#[inline]
...
fn parse_expression(state: Input) -> Output {
    state.rule(LifeRestartRule::Expression, |s| {
        parse_atomic(s).and_then(|s| s.tag_node("atomic"))
    })
}
#[inline]
fn parse_atomic(state: Input) -> Output {
    state.rule(LifeRestartRule::Atomic, |s| {
        Err(s).or_else(|s|parse_identifier(s).and_then(|s| s.tag_node("identifier"))).or_else(|s|parse_integer(s).and_then(|s| s.tag_node("integer"))).or_else(|s|parse_string_raw(s).and_then(|s| s.tag_node("string_raw"))).or_else(|s|parse_string_normal(s).and_then(|s| s.tag_node("string_normal"))).or_else(|s|parse_boolean(s).and_then(|s| s.tag_node("boolean"))).or_else(|s|parse_range_exact(s).and_then(|s| s.tag_node("range_exact"))).or_else(|s|parse_range(s).and_then(|s| s.tag_node("range")))
    })
}
#[inline]
fn parse_prefix(state: Input) -> Output {
    state.rule(LifeRestartRule::Prefix, |s| {
        Err(s).or_else(|s|builtin_regex(s,{static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^(不是|非|!)").unwrap())}).and_then(|s| s.tag_node("prefix_0")))
    })
}
#[inline]
...
fn parse_string_raw(state: Input) -> Output {
    state.rule(LifeRestartRule::StringRaw, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s,"'",false)).and_then(|s|parse_string_raw_text(s).and_then(|s| s.tag_node("string_raw_text"))).and_then(|s|builtin_text(s,"'",false)))
    })
}
#[inline]
...
fn parse_string_normal(state: Input) -> Output {
    state.rule(LifeRestartRule::StringNormal, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s,"\"",false)).and_then(|s|s.repeat(0..4294967295,|s|parse_string_item(s).and_then(|s| s.tag_node("string_item")))).and_then(|s|builtin_text(s,"\"",false)))
    })
}
#[inline]
fn parse_string_item(state: Input) -> Output {
    state.rule(LifeRestartRule::StringItem, |s| {
        Err(s).or_else(|s|parse_escaped_unicode(s).and_then(|s| s.tag_node("escaped_unicode"))).or_else(|s|parse_escaped_character(s).and_then(|s| s.tag_node("escaped_character"))).or_else(|s|parse_text_any(s).and_then(|s| s.tag_node("text_any")))
    })
}
#[inline]
fn parse_escaped_unicode(state: Input) -> Output {
    state.rule(LifeRestartRule::EscapedUnicode, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s,"\\u{",false)).and_then(|s|parse_hex(s).and_then(|s| s.tag_node("hex"))).and_then(|s|builtin_text(s,"}",false)))
    })
}
#[inline]
...
fn parse_hex(state: Input) -> Output {
    state.rule(LifeRestartRule::HEX, |s| {
        s.repeat(1..6,|s|s.sequence(|s|Ok(s).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_regex(s,{static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^([0-9a-fA-F])").unwrap())}))))
    })
}
#[inline]
...
fn parse_range_exact(state: Input) -> Output {
    state.rule(LifeRestartRule::RangeExact, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s,"{",false)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_integer(s).and_then(|s| s.tag_node("integer"))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s,"}",false)))
    })
}
#[inline]
fn parse_range(state: Input) -> Output {
    state.rule(LifeRestartRule::Range, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s,"{",false)).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|parse_integer(s).and_then(|s| s.tag_node("min")))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s,",",false)).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|parse_integer(s).and_then(|s| s.tag_node("max")))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s,"}",false)))
    })
}
#[inline]
fn parse_boolean(state: Input) -> Output {
    state.rule(LifeRestartRule::Boolean, |s| {
        Err(s).or_else(|s|builtin_regex(s,{static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^(真|true)").unwrap())}).and_then(|s| s.tag_node("true"))).or_else(|s|builtin_regex(s,{static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^(假|false)").unwrap())}).and_then(|s| s.tag_node("false")))
    })
}
#[inline]
...
fn parse_kw_trait_group(state: Input) -> Output {
    state.rule(LifeRestartRule::KW_TRAIT_GROUP, |s| {
        s.match_regex({static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^(特质组|特征组|traits|trait-group)").unwrap())})
    })
}
#[inline]
fn parse_kw_trait(state: Input) -> Output {
    state.rule(LifeRestartRule::KW_TRAIT, |s| {
        s.match_regex({static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^(特质|特征|trait)").unwrap())})
    })
}
#[inline]
fn parse_kw_event_group(state: Input) -> Output {
    state.rule(LifeRestartRule::KW_EVENT_GROUP, |s| {
        s.match_regex({static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^(事件组|events|event-group)").unwrap())})
    })
}
#[inline]
fn parse_kw_event(state: Input) -> Output {
    state.rule(LifeRestartRule::KW_EVENT, |s| {
        s.match_regex({static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^(事件|event)").unwrap())})
    })
}
#[inline]
fn parse_kw_id(state: Input) -> Output {
    state.rule(LifeRestartRule::KW_ID, |s| {
        s.match_regex({static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^(编号|[Ii][Dd])").unwrap())})
    })
}
#[inline]
...
fn parse_comment(state: Input) -> Output {
    state.rule(LifeRestartRule::Comment, |s| {
        Err(s).or_else(|s|builtin_regex(s,{static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^(\\/\\/[^\\n\\r]*)").unwrap())})).or_else(|s|s.sequence(|s|Ok(s).and_then(|s|builtin_text(s,"/*",false)).and_then(|s|builtin_text(s,"*/",false))))
    })
}
...
fn builtin_ignore(state: Input) -> Output {
    state.repeat(0..u32::MAX, |s| {
        parse_white_space(s).or_else(|s| parse_comment(s))
    })

...
pub(super) fn parse_cst(input: &str, rule: LifeRestartRule) -> OutputResult<LifeRestartRule> {
    state(input, |state| match rule {
        ...
        LifeRestartRule::TopStatement => parse_top_statement(state),
        ...
        LifeRestartRule::EffectStatement => parse_effect_statement(state),
        ...
        LifeRestartRule::KW_EFFECT => parse_kw_effect(state),
                    ...
                    s.repeat(0..4294967295, |s| {
                        s.sequence(|s| {
                            Ok(s)
                                ...
                                .and_then(|s| parse_top_statement(s).and_then(|s| s.tag_node("top_statement")))
                        })
                    })
                })
                .and_then(|s| builtin_ignore(s))
...
#[inline]
fn parse_top_statement(state: Input) -> Output {
    state.rule(LifeRestartRule::TopStatement, |s| {
        Err(s)
                ...
                .and_then(|s| {
                    s.repeat(0..4294967295, |s| {
                        s.sequence(|s| {
                            Ok(s).and_then(|s| builtin_ignore(s)).and_then(|s| {
                                Err(s)
                                    .or_else(|s| parse_trait_item(s).and_then(|s| s.tag_node("trait_item")))
...
fn parse_trait_item(state: Input) -> Output {
    state.rule(LifeRestartRule::TraitItem, |s| {
        Err(s)
            ...
            .or_else(|s| parse_description_statement(s).and_then(|s| s.tag_node("description_statement")))
            .or_else(|s| parse_requirement_statement(s).and_then(|s| s.tag_node("requirement_statement")))
            .or_else(|s| parse_effect_statement(s).and_then(|s| s.tag_node("effect_statement")))
    })
}
#[inline]
...
#[inline]
fn parse_effect_statement(state: Input) -> Output {
    state.rule(LifeRestartRule::EffectStatement, |s| {
        s.sequence(|s| {
            Ok(s)
                .and_then(|s| parse_kw_effect(s))
                .and_then(|s| builtin_ignore(s))
                .and_then(|s| builtin_text(s, "{", false))
                .and_then(|s| builtin_ignore(s))
                .and_then(|s| {
                ...
                .and_then(|s| {
                    s.repeat(0..4294967295, |s| {
                        s.sequence(|s| {
                            Ok(s).and_then(|s| builtin_ignore(s)).and_then(|s| {
                                Err(s)
                                    .or_else(|s| parse_event_item(s).and_then(|s| s.tag_node("event_item")))
                                    .or_else(|s| parse_eos(s))
                            })
                        })
                    })
...
fn parse_statement(state: Input) -> Output {
    state.rule(LifeRestartRule::Statement, |s| {
        Err(s)
            .or_else(|s| parse_if_statement(s).and_then(|s| s.tag_node("if_statement")))
            .or_else(|s| parse_expression(s).and_then(|s| s.tag_node("expression")))
            .or_else(|s| parse_eos(s).and_then(|s| s.tag_node("eos")))
    })
}
#[inline]
fn parse_if_statement(state: Input) -> Output {
    state.rule(LifeRestartRule::IfStatement, |s| {
        s.sequence(|s| {
            Ok(s)
...
#[inline]
fn parse_if_then_block(state: Input) -> Output {
    state.rule(LifeRestartRule::IfThenBlock, |s| {
        s.sequence(|s| {
            Ok(s)
...
#[inline]
fn parse_if_else_block(state: Input) -> Output {
    state.rule(LifeRestartRule::IfElseBlock, |s| {
        s.sequence(|s| {
            Ok(s)
    ...
    state.rule(LifeRestartRule::Infix, |s| {
        Err(s)
            .or_else(|s| {
                builtin_regex(s, {
                    static REGEX: OnceLock<Regex> = OnceLock::new();
                    REGEX.get_or_init(|| Regex::new("^([加+])").unwrap())
                })
                .and_then(|s| s.tag_node("add"))
            })
            .or_else(|s| {
                builtin_regex(s, {
                    static REGEX: OnceLock<Regex> = OnceLock::new();
                    REGEX.get_or_init(|| Regex::new("^(增加|\\+=)").unwrap())
                })
                .and_then(|s| s.tag_node("add_assign"))
            })
            .or_else(|s| {
                builtin_regex(s, {
                    static REGEX: OnceLock<Regex> = OnceLock::new();
                    REGEX.get_or_init(|| Regex::new("^([减\\-])").unwrap())
                })
                .and_then(|s| s.tag_node("sub"))
            })
            .or_else(|s| {
                builtin_regex(s, {
                    static REGEX: OnceLock<Regex> = OnceLock::new();
                    REGEX.get_or_init(|| Regex::new("^([减少\\-=])").unwrap())
                })
                .and_then(|s| s.tag_node("sub_assign"))
            })
            .or_else(|s| {
                builtin_regex(s, {
                    static REGEX: OnceLock<Regex> = OnceLock::new();
...
#[inline]
fn parse_kw_effect(state: Input) -> Output {
    state.rule(LifeRestartRule::KW_EFFECT, |s| {
        s.match_regex({
            static REGEX: OnceLock<Regex> = OnceLock::new();
            REGEX.get_or_init(|| Regex::new("^(效果|一次性效果|effects?)").unwrap())
        })
    })
}
#[inline]
